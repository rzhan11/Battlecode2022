...
public class Soldier extends Robot {
    ...
    public static double getSmartAttackScore(Direction dir) throws GameActionException {
        ...
        for (int j = curEnemyDangerCount; --j >= 0;) {
            ...
            if (destLoc.isWithinDistanceSquared(ri.location, myActionRadius)) { // if i can hit it
                double theirTime = (10 + rc.senseRubble(ri.location)) / 10.0;
...
//                case SAGE:
    ...
    public static Direction moveLogic() throws GameActionException {
        ...
        if (!rc.isActionReady() && closestEnemySoldier != null) {// || !checkShouldFight()) {
...
//        }
    ...
    public static Direction chaseSoldier() throws GameActionException {
        return null;
    }
    ...
    public static void updateTargetEnemyLoc() {
        ...
        if (index >= 0) {
            log("yep " + index + " " + Comms.reportedEnemyLocs[index] + " " + Comms.isReportedEnemyLocDanger[index]);
            ...
            log("ns " + newScore + " " + curDist + " " + newLoc + " " + newDanger);
            log("os " + targetEnemyLoc + " " + targetEnemyLocDanger);
